...
// Copies only supplied data. `log(earn)` repeats Stan transformed-data work
// at each evaluation; no data-only sufficient statistic is formed at bind time.
struct Data { earn: Vec<f64>, height: Vec<f64> }
struct Bound { data: Data, ndim: usize }
struct Workspace;

fn numeric_array(o: &serde_json::Map<String, Value>, key: &str) -> Result<Vec<f64>, String> {
    o.get(key).and_then(Value::as_array).ok_or_else(|| format!("{key} must be an array"))?
        .iter().map(|x| x.as_f64().filter(|v| v.is_finite())
            .ok_or_else(|| format!("{key} must contain finite numerics"))).collect()
}
fn parse_data(v: Value) -> Result<Data, String> {
    let o = v.as_object().ok_or("data must be a JSON object")?;
    let n = o.get("N").and_then(Value::as_u64).ok_or("N must be a nonnegative integer")? as usize;
    let earn = numeric_array(o, "earn")?;
    let height = numeric_array(o, "height")?;
    if earn.len() != n || height.len() != n { return Err("N/array length mismatch".into()); }
    if earn.iter().any(|x| *x <= 0.0) { return Err("earn must be positive".into()); }
    Ok(Data { earn, height })
}
...
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    ...
    let mut lp = q[2]; // lower=0 sigma log-Jacobian
    let mut gb0 = 0.0;
    let mut gb1 = 0.0;
    let mut gs_constrained = 0.0;
    for (&earn, &height) in d.earn.iter().zip(&d.height) {
        // Stan's transformed-data assignment is deliberately retained here.
        let residual = earn.ln() - (q[0] + q[1] * height);
        let z = residual * inv_sigma;
        // normal_lpdf(... | ..., sigma), propto=true: parameter-dependent terms.
        lp += -q[2] - 0.5 * z * z;
        let w = residual * inv_sigma_sq;
        gb0 += w;
        gb1 += height * w;
        gs_constrained += -inv_sigma + residual * residual * inv_sigma * inv_sigma_sq;
    }
    g[0] = gb0;
    g[1] = gb1;
    g[2] = gs_constrained * sigma + 1.0; // chain rule plus Jacobian derivative
    Ok(lp)
}
