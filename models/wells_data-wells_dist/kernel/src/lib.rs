...
// Immutable copies reproduce the original data and likelihood on every call.
struct Data { switched: Vec<u8>, dist: Vec<f64> }
...
fn parse_data(v: Value) -> Result<Data, String> {
    ...
    if switched_v.len() != n || dist_v.len() != n { return Err("data length does not equal N".into()); }
    let mut switched = Vec::with_capacity(n); let mut dist = Vec::with_capacity(n);
    for (i, x) in switched_v.iter().enumerate() {
        match x.as_u64() { Some(0) => switched.push(0), Some(1) => switched.push(1), _ => return Err(format!("switched.{} must be 0 or 1", i + 1)) }
    }
    for (i, x) in dist_v.iter().enumerate() {
        let x = x.as_f64().filter(|x| x.is_finite()).ok_or_else(|| format!("dist.{} must be finite numeric", i + 1))?;
        dist.push(x);
    }
    Ok(Data { switched, dist })
}
...
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    let (b0, b1) = (q[0], q[1]);
    if !b0.is_finite() || !b1.is_finite() { return Err("non-finite position".into()); }
    let mut lp = 0.0; let (mut gb0, mut gb1) = (0.0, 0.0);
    for (&y, &x) in d.switched.iter().zip(&d.dist) {
        let eta = b0 + b1 * x;
        if !eta.is_finite() { return Err("non-finite linear predictor".into()); }
        lp += (y as f64) * eta - softplus(eta);
        let r = (y as f64) - sigmoid(eta); gb0 += r; gb1 += r * x;
    }
    g[0] = gb0; g[1] = gb1; Ok(lp)
}
