...
struct Data { income: Vec<f64>, vote: Vec<f64> }
...
fn parse_data(v: Value) -> Result<Data, String> {
    ...
    let income_v = o.get("income").and_then(Value::as_array).ok_or("income must be an array")?;
    let vote_v = o.get("vote").and_then(Value::as_array).ok_or("vote must be an array")?;
    if income_v.len() != n || vote_v.len() != n { return Err("income/vote length must equal N".into()); }
    let mut income = Vec::with_capacity(n); let mut vote = Vec::with_capacity(n);
    for x in income_v { income.push(finite_num(x, "income")?); }
    for y in vote_v {
        match y.as_u64() { Some(0) => vote.push(0.0), Some(1) => vote.push(1.0), _ => return Err("vote must contain integers 0 or 1".into()) }
    }
    Ok(Data { income, vote })
}
...
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    ...
    for (&x, &y) in d.income.iter().zip(d.vote.iter()) {
        let eta = alpha + beta * x;
        // Exact bernoulli_logit_lpmf at propto=TRUE: y * eta - log1p_exp(eta).
        let softplus = eta.max(0.0) + (-eta.abs()).exp().ln_1p();
        lp += y * eta - softplus;
        let sigmoid = if eta >= 0.0 { 1.0 / (1.0 + (-eta).exp()) } else { let e = eta.exp(); e / (1.0 + e) };
        let residual = y - sigmoid;
        ga += residual; gb += residual * x;
    }
    g[0] = ga; g[1] = gb;
    Ok(lp)
}
...
#[no_mangle] pub unsafe extern "C" fn nutpier_density_evaluator_bind(
    ...
    let answer = catch_unwind(AssertUnwindSafe(|| -> Result<Bound, String> {
        ...
        if ndim != 2 || got != want { return Err(format!("dimension/layout mismatch: ndim={ndim}, got={got:?}, want={want:?}")); }
        Ok(Bound { data, ndim })
    }));
    match answer {
