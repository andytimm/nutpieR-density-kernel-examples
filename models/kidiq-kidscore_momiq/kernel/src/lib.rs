...
struct Data { kid_score: Vec<f64>, mom_iq: Vec<f64> }
...
fn finite_vector(o: &serde_json::Map<String, Value>, key: &str, n: usize) -> Result<Vec<f64>, String> {
    ...
    for x in a { let y = x.as_f64().filter(|z| z.is_finite() && *z >= 0.0 && *z <= 200.0)
        .ok_or_else(|| format!("{key} must contain finite values in [0, 200]"))?; out.push(y); }
    Ok(out)
}
fn parse_data(v: Value) -> Result<Data, String> {
    let o = v.as_object().ok_or("data must be a JSON object")?;
    let n_f = finite_num(&v, "N")?;
    if n_f < 0.0 || n_f.fract() != 0.0 || n_f > usize::MAX as f64 { return Err("N must be a nonnegative integer".into()); }
    let n = n_f as usize;
    Ok(Data { kid_score: finite_vector(o, "kid_score", n)?, mom_iq: finite_vector(o, "mom_iq", n)? })
}
fn expected_layout(_: &Data) -> String { "beta.1\nbeta.2\nsigma".into() }
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    if q.iter().any(|x| !x.is_finite()) { return Err("non-finite position".into()); }
    let beta0 = q[0]; let beta1 = q[1]; let log_sigma = q[2];
    let sigma = log_sigma.exp();
    if !sigma.is_finite() || sigma == 0.0 { return Err("sigma transform out of domain".into()); }
    let inv_sigma2 = 1.0 / (sigma * sigma);
    let mut lp = log_sigma; // lower=0 transform Jacobian
    let mut gb0 = 0.0; let mut gb1 = 0.0; let mut glogsigma = 0.0;
    // Original independent normal likelihood terms; no data-only reductions.
    for (&y, &x) in d.kid_score.iter().zip(&d.mom_iq) {
        let residual = y - (beta0 + beta1 * x);
        let z2 = residual * residual * inv_sigma2;
        lp += -0.5 * z2 - log_sigma;
        let scaled_residual = residual * inv_sigma2;
        gb0 += scaled_residual;
        gb1 += x * scaled_residual;
        glogsigma += z2 - 1.0;
    }
    // cauchy(0, 2.5)_lupdf(sigma), retaining its sigma-dependent term only.
    let ratio2 = sigma * sigma / 6.25;
    lp -= ratio2.ln_1p();
    glogsigma += -2.0 * ratio2 / (1.0 + ratio2);
    g[0] = gb0; g[1] = gb1; g[2] = glogsigma + 1.0; // Jacobian derivative
    Ok(lp)
}
unsafe fn bytes<'a>(p: *const c_char, n: usize) -> Result<&'a [u8], String> {
    if n == 0 { Ok(&[]) } else if p.is_null() { Err("null input".into()) } else { Ok(unsafe { slice::from_raw_parts(p.cast(), n) }) }
}
