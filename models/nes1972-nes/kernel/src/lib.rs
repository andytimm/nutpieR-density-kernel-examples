...
struct Data {
    ...
    gender: Vec<f64>, income: Vec<f64>, age: Vec<i32>,
}
struct Bound { data: Data, ndim: usize }
struct Workspace;

fn array_f64(o: &serde_json::Map<String, Value>, key: &str, n: usize) -> Result<Vec<f64>, String> {
...
fn array_i32(o: &serde_json::Map<String, Value>, key: &str, n: usize) -> Result<Vec<i32>, String> {
    let a = o.get(key).and_then(Value::as_array).ok_or_else(|| format!("{key} must be an integer array"))?;
    if a.len() != n { return Err(format!("{key} has wrong length")); }
    a.iter().map(|x| x.as_i64().and_then(|v| i32::try_from(v).ok()).ok_or_else(|| format!("{key} must be integer"))).collect()
}
fn parse_data(v: Value) -> Result<Data, String> {
    let o = v.as_object().ok_or("data must be a JSON object")?;
    let n = o.get("N").and_then(Value::as_u64).and_then(|x| usize::try_from(x).ok()).ok_or("N must be a nonnegative integer")?;
    Ok(Data { partyid7: array_f64(o,"partyid7",n)?, real_ideo: array_f64(o,"real_ideo",n)?,
        race_adj: array_f64(o,"race_adj",n)?, educ1: array_f64(o,"educ1",n)?, gender: array_f64(o,"gender",n)?,
        income: array_f64(o,"income",n)?, age: array_i32(o,"age_discrete",n)? })
}
fn expected_layout(_: &Data) -> String { (1..=9).map(|i| format!("beta.{i}")).chain(std::iter::once("sigma".to_string())).collect::<Vec<_>>().join("\n") }
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    if q.iter().any(|x| !x.is_finite()) { return Err("non-finite position".into()); }
    g.fill(0.0);
    let log_sigma = q[9]; let sigma = log_sigma.exp();
    ...
    let mut lp = log_sigma; // log Jacobian for sigma = exp(log_sigma)
    let mut gs = 1.0;
    for i in 0..d.partyid7.len() {
        let age = d.age[i];
        let x = [1.0, d.real_ideo[i], d.race_adj[i], (age == 2) as i32 as f64,
            (age == 3) as i32 as f64, (age == 4) as i32 as f64, d.educ1[i], d.gender[i], d.income[i]];
        let eta: f64 = q[..9].iter().zip(x).map(|(b,x)| b*x).sum();
        let r = d.partyid7[i] - eta;
        lp += -0.5 * r * r * inv_var - log_sigma;
        let coeff = r * inv_var;
        for j in 0..9 { g[j] += coeff * x[j]; }
        gs += r * r * inv_var - 1.0;
    }
    g[9] = gs;
    Ok(lp)
