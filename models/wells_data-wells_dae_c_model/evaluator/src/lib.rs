...
struct Data { n: usize, switched: Vec<u8>, dist: Vec<f64>, arsenic: Vec<f64>, educ: Vec<f64> }
...
fn array(v: &Value, key: &str, n: usize) -> Result<Vec<f64>, String> {
    ...
    a.iter().map(|x| num(x, key)).collect()
}
fn parse_data(v: Value) -> Result<Data, String> {
    let o = v.as_object().ok_or("data must be a JSON object")?;
    let n_f = num(o.get("N").ok_or("N missing")?, "N")?;
    if n_f < 0.0 || n_f.fract() != 0.0 || n_f > usize::MAX as f64 { return Err("N must be nonnegative integer".into()); }
    let n = n_f as usize;
    let switched_f = array(&v, "switched", n)?;
    let mut switched = Vec::with_capacity(n);
    for x in switched_f { if x != 0.0 && x != 1.0 { return Err("switched must contain 0 or 1".into()); } switched.push(x as u8); }
    Ok(Data { n, switched, dist: array(&v, "dist", n)?, arsenic: array(&v, "arsenic", n)?, educ: array(&v, "educ", n)? })
}
fn expected_layout(_d: &Data) -> String { "alpha\nbeta.1\nbeta.2\nbeta.3\nbeta.4".into() }
fn softplus(x: f64) -> f64 { if x > 0.0 { x + (-x).exp().ln_1p() } else { x.exp().ln_1p() } }
fn inv_logit(x: f64) -> f64 { if x >= 0.0 { 1.0 / (1.0 + (-x).exp()) } else { let e=x.exp(); e/(1.0+e) } }
fn eval_model(d: &Data, q: &[f64], g: &mut [f64]) -> Result<f64, String> {
    // This repeats the original Stan transformed-data expressions per evaluation.
    let mut mean_dist = 0.0;
    let mut mean_arsenic = 0.0;
    for i in 0..d.n { mean_dist += d.dist[i]; mean_arsenic += d.arsenic[i]; }
    mean_dist /= d.n as f64; mean_arsenic /= d.n as f64;
    g.fill(0.0);
    let (alpha, b1, b2, b3, b4) = (q[0], q[1], q[2], q[3], q[4]);
    let mut lp = 0.0;
    for i in 0..d.n {
        let c_dist100 = (d.dist[i] - mean_dist) / 100.0;
        let c_arsenic = d.arsenic[i] - mean_arsenic;
        let da_inter = c_dist100 * c_arsenic;
        let educ4 = d.educ[i] / 4.0;
        let eta = alpha + c_dist100*b1 + c_arsenic*b2 + da_inter*b3 + educ4*b4;
        let y = d.switched[i] as f64;
        lp += y * eta - softplus(eta);
        let r = y - inv_logit(eta);
        g[0] += r; g[1] += r*c_dist100; g[2] += r*c_arsenic; g[3] += r*da_inter; g[4] += r*educ4;
    }
    Ok(lp)
}
