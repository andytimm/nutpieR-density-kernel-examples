...
random <- nutpie_validate_density_kernel(bound)
print(random)
if (!identical(random$status, "pass")) stop("default density kernel check failed", call. = FALSE)

if (run_reference) {
  reference_check <- nutpie_validate_density_kernel(bound, method = "reference")
  print(reference_check)
  if (!identical(reference_check$status, "pass")) {
    stop("reference density kernel check failed", call. = FALSE)
  }
}
